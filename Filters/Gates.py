...
def make_translation(num_summing, num_carry, auxiliary, skipped): #HEREE
    ...
    print("defined qubits for gate: "+str(num_summing)+" for original "+str(num_summing) +" for shift "+ str(num_carry)+" for carry")
    for index in range(num_summing):
        translation.compose(carry_gate,
                            [index + 2 * num_summing, index, index + num_summing, index + 2 * num_summing + 1],
                            inplace=True)
        # translation.cx(index,index+num_summing)
    print("carry gates added")
    ...
    for index in reversed(range(num_summing)):
        ...
        if index != num_summing - 1:
            ...
            translation.compose(carry_gate.inverse(), [carry_sum, add_1, add_2, carry_post], inplace=True)
...
def encode_number(quantumImage, shift):
    ...
    for index, element in enumerate(number[::-1]):
        if element == '1':
            ...
            quantumImage.circuit.x(quantumImage.total_qubits + quantumImage.n_aux_qubit + index)  # da fare fuori


def traslate_circuit(quantumImage, axis, shift, skip=False):
    print("encoding")
    ...
    print("#1")
    if axis == 'x':
        control_qubits = control_qubits + [i for i in range(quantumImage.n_aux_qubit,
                                                            quantumImage.n_aux_qubit + quantumImage.num_summing)]
        print("#2")
    elif axis == 'y':
        control_qubits = control_qubits + [i for i in range(quantumImage.n_aux_qubit + quantumImage.num_summing,
                                                            quantumImage.n_aux_qubit + 2 * quantumImage.num_summing)]
        print("#2")
    control_qubits = control_qubits + [i for i in range(quantumImage.n_aux_qubit + quantumImage.total_qubits,
                                                        quantumImage.n_aux_qubit + quantumImage.total_qubits + quantumImage.num_summing + quantumImage.num_carry)]
    print("#3")
    quantumImage.circuit.append(
        make_translation(quantumImage.num_summing, quantumImage.num_carry, quantumImage.n_aux_qubit, skip),
        control_qubits)
    print("#4")
...
def adder(circuit, wires, shift):
    ...
    for index, wire in enumerate(wires):
        ...
        circuit.compose(carry_gate, [carry_pre, first, adder, carry_post], inplace=True)
        circuit.barrier()
    ...
    for index, wire in reversed(list(enumerate(wires))):
        ...
        if index != len(wires) - 1:
            carry_post = carry_sum + 1
            circuit.compose(carry_gate.inverse(), [carry_sum, add_1, add_2, carry_post], inplace=True)
            circuit.barrier()
